	...
	<view v-else class="navbar-wrapper">
		...
		<view class="navbar" :class="{ fixed }" :style="{ height: totalHeight + 'px' }">
			...
			<view class="nav-content" :style="{ height: navHeight + 'px' }">
				...
				<!-- 左侧 -->
				<view class="nav-left">
					<image v-if="leftIcon" :src="leftIcon" class="icon" @click="goUrl" />
					<image v-else-if="showBack" src="/static/icon/back.png" class="icon" @click="goUrl" />
				</view>

				<!-- 标题（绝对居中） -->
				<view v-if="!onlyBack" class="nav-title">
					<text class="title-text" :class="titleClass" :style="titleStyle">
						{{ title }}
					</text>
