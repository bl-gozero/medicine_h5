	...
	<view class="page bg-white plr-20">
		...
		<view v-if="type == 1" class="">
			...
			<view v-if="payList.length > 0" class="">
				<view class="flex-between item-stretch" v-for="item in payList" :key="item.id"
					@click="group = item;showOperation = true">
					<view class="ptb-10">
						<view class="relative">
							<u-avatar :src="item.icon" size="42" default-url="/static/group/default.png"
								mode="aspectFill"></u-avatar>
							<view v-if="item.role && item.role.id < 3"
					...
					<view class="border-bottom flex-between ml-8 flex-1">
						...
						<view v-if="item.is_payment && item.is_payment.id == 2"
							class="bg-base text-white fs-12 lh-8 fw-5 w-53 h-21 rounded-x flex-center plr-0"
							shape="circle">{{ item.is_payment.value }}</view>
					</view>
				</view>
			</view>
			<view class="">
				<view v-if="item.is_payment && item.is_payment.id == 1" class="flex-between item-stretch"
					v-for="item in list" :key="item.id" @click="onGroup(item)">
					<view class="ptb-10">
						<view class="relative">
							<u-avatar :src="item.icon" size="42" default-url="/static/group/default.png"
								mode="aspectFill"></u-avatar>
					...
					<view class="border-bottom flex-between ml-8 flex-1">
						...
						<view v-if="item.is_verify && item.is_verify.id == 1"
							class="text-white fs-12 lh-8 w-53 h-21 rounded-x flex-center ml-8"
							style="background: #D6D6D6;">审核中</view>
					</view>
				</view>
			</view>
		</view>
		<view v-if="type == 2" class="">
			...
			<view v-else v-for="item in friendList.list" :key="item.id" @click="onFriend(item)">
				<view v-if="!item.isBlocked" class="flex-between item-stretch">
					<view class="ptb-10">
						<u-avatar :src="$c.formatImgUrl(item.userProfile? item.userProfile.avatar : '')"
							:defaultUrl="$c.userAvatar()" size="42" mode="aspectFill"></u-avatar>
					</view>
		...
		<u-popup :show="showOperation" mode="bottom" round="20" @close="showOperation = false">
			<view class="plr-20 pt-5 pb-50">
				<view class="text-center">
					<view class="ptb-20 border-bottom"
	...
	export default {
		...
		methods: {
			...
			async onDelete() {
				...
				if (this.type == 1) {
					const res = await this.$c.fetch(this.$api.group.payDelete, {
						id: this.group.id
					})
					if (res) {
						this.getPayList()
