<template>
	<view class="page bg-white">
		...
		<view class="plr-20 pt-10">
			...
			<view class="mt-10">
				<view class="flex-between item-stretch" v-for="item in list" :key="item.account_id"
					@click="onSelect(item)">
					...
					<view class="flex-1 flex-between border-bottom ml-8">
						...
						<u-icon v-if="item.role && (item.role.id == 1 || item.role.id == 2)"
							name="checkmark-circle-fill" color="#ECB4A8" size="20"></u-icon>
						<u-icon v-else-if="item.isSele" name="checkmark-circle-fill" :color="$c.baseColor()"
							size="20"></u-icon>
