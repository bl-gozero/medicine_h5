<template>
	<view class="page bg-white">
				...
				<view v-show="nav == 1 || (item.chat_banned && item.chat_banned.id == 1)" class="flex-between item-stretch" v-for="item in list" :key="item.account_id">
					...
					<view class="flex-1 flex-between border-bottom ml-8">
						...
						<view v-if="memberInfo.memberRole > 0 && item.role.id > 1" class="flex-end">
							<u-button
								class="w-47 h-20 fs-10 plr-0 text-danger border-danger ml-8"
								shape="circle"
								@click="onShowRemove(item)"
							>移除</u-button>
							
							<u-button
								v-if="item.chat_banned.id == 1"
								class="w-47 h-20 fs-10 plr-0 ml-8"
								shape="circle"
								@click="doBanned(item)"
							>解除禁言</u-button>
							
							<u-button
								v-if="item.chat_banned.id == 2"
								class="w-47 h-20 fs-10 plr-0 border-1 text-base ml-8"
								shape="circle"
								@click="doBanned(item)"
							>禁言</u-button>
						</view>
		...
		<u-modal :show="show" title="是否移除" content='移除后该成员将离开当前群聊？' confirmColor="#3D3D3D" cancelColor="#9F9F9F" showCancelButton
			@cancel="show = false" @confirm="doRemove"></u-modal>
	</view>
</template>

<script>
	import Title from '@/components/Title.vue'
	import { teamInfo, setMemberBanned, memberInfo, kickMember } from '../../../utils/nim'
	...
	export default {
		...
		computed: {
			...
			navList() {
				...
				return [{ id: 1, name: `全部(${teamInfo.memberCount || 0})` }, { id: 2, name: `已禁言(${banned})` }]
			}
		},
		onLoad() {
			this.$c.checkeLogin()
				...
				if (res1) {
					...
					if (res) {
						this.list
							...
							.map(i => {
								i.chat_banned = chat_banned == 1? { id: 1, value: '是' } : { id: 2, value: '否' }
							})
					}
				}
			},
