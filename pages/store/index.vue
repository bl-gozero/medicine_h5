<template>
	<view class="page bg-page">
		...
		<view v-if="nav == 1" class="total_box fixed pw-100 bottom-0 left-0 plr-20 ptb-9 flex-between fs-10 border-box bg-white"
			style="z-index: 10;">
			<view class="">
		...
		<u-popup :show="showTransfer" mode="bottom" bgColor="transparent" closeable @close="showTransfer = false">
			<view class="pt-14 pb-30 plr-20 bg-address lh-10 roundedTop-20">
				...
				<view class="ptb-20 flex-between border-bottom border-top mt-20">
					...
					<u-input v-model="to_account" placeholder="手机号/账号" placeholderClass="fs-14 text-info"
	...
	export default {
		...
		methods: {
			...
			onShowEvent(e) {
				...
				if(e == 'buy')  this.$c.toast('未到开放时间') //this.showBuy = true
			},
			init() {
				if(this.status == 'load') return
				this.status = 'more'
