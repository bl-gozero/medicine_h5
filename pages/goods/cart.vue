<template>
	<view class="page bg-page">
		...
		<view class="pt-11 plr-8">
			<view class="ptb-14 plr-12 rounded-12 flex-start bg-white" v-for="item in list" :key="item.id">
		...
		<view class="total_box fixed pw-100 bottom-48 left-0 plr-20 ptb-9 flex-between fs-10 border-box bg-white" style="z-index: 10;">
			...
			<view v-if="mode == 1" class="flex-end">
				...
				<button class="bg-base-change fw-7 fs-14 text-white w-82 h-35 rounded-x">结算</button>
			</view>
			<view v-else class="">
				<button class="bg-danger fw-7 fs-14 text-white w-82 h-35 rounded-x" @click="onShowDelete()">删除</button>
			</view>
		</view>
		<u-modal :show="show" title="提示" content='确定要删除？' showCancelButton @click="onDelete()"></u-modal>
		<TabBar />
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			async getList() {
				...
				if(res) { res.map(item => { item.status = false }); this.list = res }
			},
			async getProfile() {
			...
			async onEdit(e) {
				const res = await this.$c.fetch(this.$api.goods.cartEdit, { id: e.name, quantity: e.value })
			},
			onShowDelete() {
			...
			async onDelete() {
				...
				const res = await this.$c.fetch(this.$api.goods.cartDelete, { id: ids })
				if(res) this.getList()
			},
		}
	} 
