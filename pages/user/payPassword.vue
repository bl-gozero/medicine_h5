<template>
	<view class="page bg-page">
		...
		<view class="mt-20 plr-20">
			...
			<view class="mt-7 bg-white rounded-8 pb-7 plr-16">
				<LineInput v-model="form.password" type="password" placeholder="请输入6位数字"
					placeholderClass="text-info fs-14 fw-7" :maxlength="6" formatter="number" />
			</view>

			<view class="title mt-40">{{ type == 1? '确认交易密码' : '确认新密码' }}</view>
			<view class="mt-7 bg-white rounded-8 pb-7 plr-16">
				<LineInput v-model="form.confirm_password" type="password" placeholder="请再次填写确认"
					placeholderClass="text-info fs-14 fw-7" :maxlength="6" />
			</view>
