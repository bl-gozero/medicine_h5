				...
				<view class="bg-white plr-14 rounded-8 mt-15" v-for="item in list" :key="item.id">
					...
					<view class="ptb-15 flex-between">
						<view class="flex-start">
							<u-icon v-if="item.is_default" name="checkmark-circle-fill" :color="$c.baseColor()"
								size="20"></u-icon>
			...
			<view class="plr-20">
				<view class="bg-white rounded-8 plr-14">
					<view class="ptb-18 border-bottom flex-between">
						...
						<u-input v-model="form.name" placeholder="请填写联系人姓名" placeholderClass="text-info fs-14"
							inputAlign="right" border="none"></u-input>
					</view>
					<view class="ptb-18 flex-between">
						<text class="mr-20">手机号码</text>
						<u-input v-model="form.phone" placeholder="请填写联系人手机号码" placeholderClass="text-info fs-14"
							inputAlign="right" border="none"></u-input>
					</view>
				</view>
				<view class="bg-white rounded-8 plr-14 mt-11">
					<view class="ptb-18 border-bottom flex-between">
						...
						<RegionPicker v-model="form.district" placeholder="请填写省市区街道"></RegionPicker>
					</view>
					<view class="ptb-18 flex-between">
						<text class="mr-20">详细地址</text>
						<u-input v-model="form.address" placeholder="请输入详细地址" placeholderClass="text-info fs-14"
							inputAlign="right" border="none"></u-input>
					</view>
				</view>
				<view class="bg-white rounded-8 plr-14 mt-11">
					<view class="ptb-18 flex-between">
						...
						<u-switch v-model="form.is_default" :activeColor="$c.baseColor()" inactiveColor="#DFDFDF"
...
<script>
	...
	export default {
		...
		methods: {
			...
			async onDelete(id) {
				const res = await this.$c.fetch(this.$api.user.addressDelete, {
					id: id
				})
				if (res) {
					this.$c.toast('删除成功')
