		...
		<view class="bg" style="min-height: 100vh;">
			...
			<view class="flex-center mt-10">
				<view class="relative w-335">
					<image :src="`/static/vip/vip/bg-lv-${$c.calcLv(profile)}.webp`" class="pw-100" mode="widthFix">
					</image>
					<view class="full border-box flex-start ph-85" style="padding-left: 8%;">
						<view class="">
							<image :src="`/static/vip/name_level_${$c.calcLv(profile)}.webp`" class="h-20 block"
								mode="heightFix"></image>
							<view class="pl-28 mt-8 fs-12">{{profile.account}}的当前等级</view>
						</view>
					</view>
					<image v-if="profile.level.id < 3" src="/static/vip/invite.webp" class="w-140 h-48 absolute right-0"
						style="bottom: 3%;" @click="$c.goto('/pages/user/team')"></image>
				</view>
			</view>
					...
					<view v-if="profile.level.id < switcher" class="plr-30">
						...
						<view class="step_box flex-between">
							...
							<u-button v-if="profile.level.id < 3" class="btn bg-0 text-0" shape="circle"
								:text="num1 > 0? `还需${num1}人` : '去完成'"
								@click="$c.goto('/pages/user/qrcode')"></u-button>
							<u-button v-else class="btn bg-1 text-1" shape="circle" text="已完成"></u-button>
						</view>
						<view v-if="switcher == 4" class="step_box flex-between step_3">
							<view class="i-35 relative">
								...
								<image src="/static/vip/good-1.webp" class="w-109 h-31 absolute bottom-30 left-20">
								</image>
							</view>
							<view class="mlr-9 flex-1">
						...
						<view class="absolute text-center pw-100 left-0 bottom-12 fs-10">
							<view>
								...
								<text class="text-base"
									@click="$c.goto('/pages/index/protocols?type=' + (switcher - 1))">
					...
					<view v-else class="flex-center">
						<view class="relative">
							...
							<view v-if="switcher == 3" class="absolute right-0 pw-57" style="top: -10%;">
								<PlayImg path="user/vip/tag_3/1" :interval="50" :length="40" type='webp' />
							</view>
							<view v-if="switcher == 4" class="absolute right-0 pw-55" style="top: -8%;">
								<PlayImg path="user/vip/tag_4/2" :interval="50" :length="40" type='webp' />
							</view>
							<view class="absolute text-center pw-100 left-0 bottom-12 fs-10">
			...
			<view v-if="profile.level.id >= 3 && performance.load && switcher == 3"
			...
			<view v-if="profile.level.id >= 4 && performance.load && switcher == 4"
				...
				<view class="partner_box pb-20 pt-10">
					<view class="">
						<swiper class="h-220" :interval="5000" :duration="500" @change="onParnterChange">
							<swiper-item v-for="item in partner_list">
								<view class="">
									<image :src="`/static/vip/partner/pointer-${item.id}.webp`" class="pw-100 maxh-50"
										mode="widthFix"></image>
									<view class="flex-center">
										<view class="relative">
											<image :src="`/static/vip/partner/bg-${item.id}.webp`"
												class="w-334 h-148 block"></image>
											<view class="absolute left-0 bottom-0 pw-100 ph-85 pl-20 pt-14 border-box">
												<view class="flex-start pw-100">
													<image :src="`/static/vip/partner/name-${item.id}.webp`"
														class="w-120 h-22"></image>
													<view v-if="item.count < item.num"
														class="unfinished text-black ml-10">未完成</view>
												</view>
												<view class="text-white fw-7 fs-12 pw-100 mt-10">升级进度</view>
												<view :class="`requirement_${item.id}`">
													<view class="flex-start mt-2">
														<view class="w-97">
															<u-line-progress :percentage="getPercent(item)"
																:showText="false" inactiveColor="#D8D8D8"
																:height="2"></u-line-progress>
														</view>
														<text
													...
													<view class="flex-start">
														<view
															class="fs-10 flex-start ptb-5 plr-7 rounded-x num relative"
															style="background: rgba(255, 255, 255, 0.5);">
															<image src="/static/vip/partner/ring.webp" class="i-14">
															</image>
					...
					<view class="mt-20 pl-20">
						<scroll-view ref="scrollView" scroll-x class="scroll-view_H" scroll-with-animation>
							<image :src="`/static/vip/privilege/${partner_index}.webp`" style="width: 1095px;"
								class="h-55 inline-block mr-20"></image>
						</scroll-view>
					</view>
				</view>
			</view>
			<view v-if="(profile.level.id == 3 && switcher == 4) || profile.level.id < 3"
				class="mt-12 roundedTop-20 form_box text-center">
	...
	export default {
		...
		methods: {
			...
			async onPay() {
				...
				const res = await this.$c.fetch(this.$api.user.upgradeVip, {
