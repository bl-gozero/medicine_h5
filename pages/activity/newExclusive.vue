<template>
	<view class="bg-white">
		...
		<u--image :src="$c.img('/static/avtivity/new/detail.webp')" width="100%" height="auto" bgColor="transparent"
			mode="widthFix">
		...
		<button v-else-if="done === 2" class="fs-14 btn rounded-x flex-center bg-audit" @click="onDetail()">已领取
			查看领取详情</button>
		...
		<u-popup :show="showDone" mode="center" bgColor="transparent" :closeOnClickOverlay="false"
			...
			<view class="w-336 text-center">
				<image :src="$c.img('/static/avtivity/new/done.webp')" class="w-336 h-364" @click="onDetail()"></image>
				<image :src="$c.img('/static/icon/close.webp')" class="i-52 mt-25" @click="showDone = false"></image>
			</view>
		</u-popup>
	</view>
</template>
