<template>
	<view class="page" style="background: #E54C32;">
			...
			<view class="flex-center" style="margin-top: -1px;">
				<view class="relative w-343">
					<view class="w-343 bg-white plr-20 pb-30 border-box" style="border-radius: 0 0 20px 20px;">
						<view class="warn_box">
							<image :src="$c.img('/static/avtivity/egg/warn.webp')" class="i-12"></image>
							<text class="ml-3">礼品若遇缺货，公司将以同等级别其它品牌作为平替，确保您的权益不受影响。</text>
						</view>
						<view class="">
							<view class="mt-6 flex-between plr-4 ptb-4 border-box rounded-8"
								style="background: #E8F1FC;" v-for="(item, i) in list" :key="item.datetime">
								<image :src="$c.img('/static/avtivity/egg/list_2.webp')" class="i-47"></image>
								<view class="flex-1 mlr-4">
									<view class="">洗衣液一桶/东北大米一箱</view>
									<view class="mt-6 text-info fs-12 mt-6">{{ done ? `第${i+1}月` : '待购物' }}</view>
								</view>
								<image v-if="item.is_expired" :src="$c.img('/static/avtivity/egg/btn-5.webp')"
									class="w-95 h-54"></image>
								<image v-else-if="item.is_participate" :src="$c.img('/static/avtivity/egg/btn-4.webp')"
									class="w-95 h-54" @click="onDetail(item)"></image>
								<image v-else :src="$c.img('/static/avtivity/egg/btn-3.webp')" class="w-95 h-54"
									@click="onAddress(item, i)"></image>
							</view>
						</view>
					</view>
				</view>
					...
					<view class="flex-between item-stretch mt-30 " v-for="item in items" @click="onGoods(item)">
						...
						<view class="flex-1 ml-8">
							<view class="flex-between">
								...
								<image :src="$c.img($c.checkIcon(select == item.id))" class="i-18"></image>
							</view>
		...
		<u-popup :show="showActive" mode="center" bgColor="transparent" :closeOnClickOverlay="false" @close="close">
			<view class="w-308 h-354 rounded-20 plr-14 border-box"
				style="background: linear-gradient(180deg, #FFEFCE 3%, #FFFFFF 49%);">
				<view class="text-center" style="margin-top: -50px;">
					<image :src="$c.img('/static/avtivity/egg/active.webp')" class="w-95 h-89" />
				</view>
				...
				<view class="flex-center fgap-10 mt-20">
					<view class="">
						<button class="w-135 h-51 fw-7 fs-16 rounded-x flex-center border-plain" plain=""
							@click="showActive = false">取消</button>
					</view>
					<view class="">
						<button class="w-135 h-51 bg-base text-white fw-7 fs-16 rounded-x flex-center"
							@click="$c.goto('/pages/goods/searchResult?is_level_valid=1')">去购买</button>
					</view>
				</view>
			</view>
		</u-popup>
